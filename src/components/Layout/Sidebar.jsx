    ...
    >
      ...
      <div className="border-t border-slate-700 px-2 py-3 space-y-1">
        ...
        <button
          ...
          title={{ light: 'Light mode (click for dark)', dark: 'Dark mode (click for system)', system: 'System mode (click for light)' }[state.themeMode]}
        ...
        >
          ...
          {!collapsed && (
            <span>{{ light: 'Light Mode', dark: 'Dark Mode', system: 'System Mode' }[state.themeMode]}</span>
          )}
        </button>

        {/* Collapse toggle */}
        <div className={`flex ${collapsed ? 'justify-center' : 'items-center justify-between px-2'}`}>
          {!collapsed && <p className="text-slate-500 text-xs">v2.008</p>}
          <button
