...
export default function SuperAdminDashboard() {
  ...
  const [activeView, setActiveView] = useState('companies'); // 'companies' | 'tests' | 'database'
    ...
    return (
      <div>
        <div className="bg-white border-b border-slate-200 px-6 py-3 flex items-center gap-3">
          <button onClick={() => setActiveView('companies')}
        ...
        <DatabaseSetup onComplete={() => { reinitialize(); setActiveView('companies'); }} />
      </div>
    );
  }
      ...
      {showReset && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6">
            ...
            <h2 className="text-lg font-semibold text-slate-800 text-center mb-2">Reset Database</h2>
            <p className="text-sm text-slate-600 text-center mb-4">
              This will permanently delete <strong>ALL</strong> data — companies, users, projects, and tasks.
              The app will return to the database setup screen. This cannot be undone.
            </p>
            {resetError && (
              <p className="text-sm text-red-600 bg-red-50 border border-red-200 rounded-lg px-3 py-2 mb-4">{resetError}</p>
            )}
            <div className="flex justify-end gap-3">
              ...
              >
                {resetting && <div className="w-3.5 h-3.5 rounded-full border-2 border-white border-t-transparent animate-spin" />}
