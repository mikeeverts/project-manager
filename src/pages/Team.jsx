...
function MemberForm({ member, departments, onSave, onClose }) {
  ...
  function handleSubmit(e) {
    ...
    onSave({ name: name.trim(), email: email.trim(), role, departmentId: departmentId || null, avatarColor });
  }
...
export default function Team() {
  ...
  function handleCreateDept(data) {
    dispatch({ type: 'ADD_DEPARTMENT', payload: { ...data, id: uuidv4(), createdAt: new Date().toISOString() } });
    setCreateDept(false);
  }

  function handleEditDept(data) {
    dispatch({ type: 'UPDATE_DEPARTMENT', payload: { ...editDept, ...data } });
    setEditDept(null);
  }
  ...
  return (
    <div className="p-6 space-y-8">
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-sm text-slate-500">{state.departments.length} department{state.departments.length !== 1 ? 's' : ''}</p>
          </div>
          <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-3">
          {state.departments.map(dept => {
            const memberCount = state.teamMembers.filter(m => m.departmentId === dept.id).length;
            const taskCount = state.tasks.filter(t => t.departmentId === dept.id).length;
            return (
              <div key={dept.id} className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                ...
                <div className="p-4 flex items-center justify-between gap-2">
                  <div className="flex items-center gap-3 min-w-0">
                    <div className="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0" style={{ backgroundColor: dept.color + '20' }}>
                      <div className="w-3 h-3 rounded-full" style={{ backgroundColor: dept.color }} />
                    </div>
                    <div className="min-w-0">
                      <p className="font-semibold text-sm text-slate-800 truncate">{dept.name}</p>
                      <p className="text-xs text-slate-400">{memberCount} member{memberCount !== 1 ? 's' : ''} · {taskCount} task{taskCount !== 1 ? 's' : ''}</p>
                    </div>
                  </div>
                  <div className="flex gap-1 flex-shrink-0">
                    <button onClick={() => setEditDept(dept)} className="p-1.5 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-lg">
                      <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                      </svg>
                    </button>
                    <button onClick={() => setDeleteDept(dept)} className="p-1.5 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-lg">
                      <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          {state.departments.length === 0 && (
            <div className="col-span-4 text-center py-8 text-slate-400 text-sm border-2 border-dashed border-slate-200 rounded-xl">
      ...
      {/* Department modals */}
      <Modal isOpen={createDept} onClose={() => setCreateDept(false)} title="New Department">
        <DepartmentForm onSave={handleCreateDept} onClose={() => setCreateDept(false)} />
      </Modal>
      <Modal isOpen={!!editDept} onClose={() => setEditDept(null)} title="Edit Department">
        {editDept && <DepartmentForm department={editDept} onSave={handleEditDept} onClose={() => setEditDept(null)} />}
      </Modal>
      <Modal isOpen={!!deleteDept} onClose={() => setDeleteDept(null)} title="Delete Department" size="sm">
        {deleteDept && (
          <div>
            <p className="text-slate-600 mb-2">Delete <strong>{deleteDept.name}</strong>?</p>
            <p className="text-slate-500 text-sm mb-6">Members will be unassigned from this department. Tasks assigned to this department will become unassigned.</p>
            <div className="flex justify-end gap-3">
              <button onClick={() => setDeleteDept(null)} className="px-4 py-2 text-sm text-slate-700 bg-slate-100 hover:bg-slate-200 rounded-lg">Cancel</button>
              <button onClick={() => handleDeleteDept(deleteDept.id)} className="px-4 py-2 text-sm text-white bg-red-600 hover:bg-red-700 rounded-lg">Delete</button>
            </div>
          </div>
        )}
      </Modal>
      ...
      <Modal isOpen={!!deleteMember} onClose={() => setDeleteMember(null)} title="Remove Member" size="sm">
        {deleteMember && (
          <div>
            <p className="text-slate-600 mb-6">Remove <strong>{deleteMember.name}</strong> from the team? Their tasks will be unassigned.</p>
            <div className="flex justify-end gap-3">
              <button onClick={() => setDeleteMember(null)} className="px-4 py-2 text-sm text-slate-700 bg-slate-100 hover:bg-slate-200 rounded-lg">Cancel</button>
              <button onClick={() => handleDeleteMember(deleteMember.id)} className="px-4 py-2 text-sm text-white bg-red-600 hover:bg-red-700 rounded-lg">Remove</button>
            </div>
          </div>
        )}
      </Modal>
