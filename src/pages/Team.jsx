  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <p className="text-slate-500 text-sm">{state.teamMembers.length} member{state.teamMembers.length !== 1 ? 's' : ''}</p>
        <button
      ...
      <Modal isOpen={createModal} onClose={() => setCreateModal(false)} title="Add Team Member">
        <MemberForm onSave={handleCreate} onClose={() => setCreateModal(false)} />
      </Modal>
      <Modal isOpen={!!editMember} onClose={() => setEditMember(null)} title="Edit Team Member">
        {editMember && <MemberForm member={editMember} onSave={handleEdit} onClose={() => setEditMember(null)} />}
      </Modal>
      <Modal isOpen={!!deleteMember} onClose={() => setDeleteMember(null)} title="Remove Member" size="sm">
        {deleteMember && (
          <div>
            <p className="text-slate-600 mb-6">
              Remove <strong>{deleteMember.name}</strong> from the team? Their tasks will be unassigned.
            </p>
            <div className="flex justify-end gap-3">
              ...
              <button onClick={() => handleDelete(deleteMember.id)} className="px-4 py-2 text-sm text-white bg-red-600 hover:bg-red-700 rounded-lg">Remove</button>
            </div>
          </div>
        )}
      </Modal>
